		...
		<div class="my-10 flex flex-col items-center justify-center gap-y-5">
			...
			<span
				>or already have an account? <a href="/login" class="group relative no-underline">
					Login
					<span
						class="absolute bottom-0 left-0 h-[2px] w-full origin-right scale-x-0 rounded-sm bg-uphButton text-white transition-transform group-hover:origin-left group-hover:scale-x-100"
					></span>
				</a></span
			>
		</div>
